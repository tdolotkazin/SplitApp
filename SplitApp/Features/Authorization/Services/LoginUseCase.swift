...
final class LoginUseCase {
    ...
    func execute(provider: AuthProvider, vc: UIViewController) async throws -> AuthResponse {

        let authResponse = try await service.login(provider: provider, vc: vc)

        TokenStore.shared.accessToken = authResponse.accessToken
                secureStorage.save(authResponse.refreshToken, for: "refresh_token")

        //secureStorage.save(authResponse.accessToken, for: "access_token")
        //secureStorage.save(authResponse.refreshToken, for: "refresh_token")
        return authResponse
